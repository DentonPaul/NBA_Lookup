...
def create():
    ...
    sql_general = f"select * from teams where teams.TeamName='{form.TeamName.data}'"
    df = pd.read_sql_query(sql_general, db.get_engine())
    general_rows = [row for index, row in df.iterrows()]

    sql_stats = f"select * from teamstats where teamstats.TeamID=(select TeamID from teams where teams.TeamName='{form.TeamName.data}')"
